...
- Convert a session's prep notes (a notebook page under the root `Sessions/` folder) to a PDF for offline reading
...
> Session markdown now lives as a notebook page under the root `Sessions/` folder (`hotd_notebook_pages.content`), not in files or the `hotd_sessions` table. The easiest path is the **notebook "Create PDF" button** shown for `Sessions/` pages in the Campaign Notebook editor (`/dm-admin#notes`), which runs this script for you (`POST /api/dm-admin/notebook/session-pdf`, GM Guide = the page minus its `# Session Summary`). To run the CLI directly, export the page content to a tmpfile and pass `--input-file` (the old positional `sessionNN` form read the now-removed `src/hotd-campaign/sessions/sessionNN.md` files).

```bash
# Export the session page content from the notebook (filenames zero-padded to 3 digits), then build the PDF
psql -h localhost -p 30432 -U cortana -d dnd_website -t -A \
  -c "SELECT content FROM hotd_notebook_pages WHERE path LIKE 'Sessions/Session 029 %'" > /tmp/session29.md
...
1. Reads the session markdown — from `--input-file` (e.g. a tmpfile exported from a notebook `Sessions/` page's `content`). The legacy positional form (`build-session-pdf.js 29`) read `src/hotd-campaign/sessions/sessionNN.md`, which no longer exists.
...
- Session source: a notebook `Sessions/` page's `content` (export to a tmpfile and pass `--input-file`; the notebook "Create PDF" button for session pages does this automatically via `POST /api/dm-admin/notebook/session-pdf`)
...
The PDF is for the DM, not the players. It can contain `dm_notes` and secret content. Do not generate a "player-facing PDF" from this skill unless the user explicitly asks; player recaps go through the [session-summary](../session-summary/SKILL.md) skill and write to the page's `# Session Summary` section.
