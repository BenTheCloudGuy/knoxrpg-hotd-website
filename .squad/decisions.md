...
Campaign **lore markdown** now lives in the DB-backed Campaign Notebook (`hotd_notebook_pages`) under the `Campaign Data/` root (top-level pages + `Groups/` + `Realms/`). It was migrated out of `src/hotd-campaign/data/*.md`. The public `/realms`, `/groups`, `/house-rules`, `/overcasting`, `/circle-magic`, and `/history` pages render from the notebook (with `## DM Notes` stripped for non-admins). Notebook pages carry a `status` (`draft`|`published`); only `published` pages are embedded into RAG. The repo `src/hotd-campaign/data/` and `src/hotd-campaign/images/` directories have now been REMOVED. NPCs are managed solely in `hotd_npcs` via the DMCC NPCs panel (`/dm-admin#npcs`); `npcs.json` and its sync workflow (`sync-npcs.js` / `sync-npcs.yml`) were retired. Images live on the uploads PVC/NAS/blob served at `/hotd-content/images/*`.
