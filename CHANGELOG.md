...
### Added
- **Printable item cards (`scripts/build-item-cards-pdf.js`).** New generator that lays out D&D item cards at baseball-card size (2.5" x 3.5"), 9-up in a 3x3 grid on US Letter or A4, as a print-ready PDF. Each item renders a front (item-name banner, centered art, rarity/type badge) and a back (stat line of rarity, type, attunement, source, followed by the description), styled with a universal parchment-and-gilt fantasy theme (aged-paper fill, leather frame, dark banner with gold lettering, gilded divider rules, sepia ink) applied uniformly to every card rather than per-rarity colors. The back page mirrors each row's columns so a duplex flip on the long edge lands every back behind its own front for clean cutting; hairline cut guides sit on every card edge and outer crop marks aid trim registration. Item data comes from the exported markdown under `items/` (YAML front-matter + body); art is pulled from `items/magic-items/images/<base-slug>.png` and embedded as base64 so the rendered HTML is self-contained. Item slugs can be passed as positional args, `--items a,b,c`, or `--from-file`; more than 9 items paginate into additional sheet pairs. With no args it auto-selects 9 items that have artwork (one per name family) as a demo written to `reports/item-cards.pdf`. Options: `--out`, `--title`, `--size letter|a4`, `--keep-html`. Rendered with weasyprint; the layout uses inline-block tiling and fixed-height table-cell bands because the installed weasyprint (61.1) has no CSS grid and unreliable flex growth with images.
- **AI-condensed card descriptions.** Item descriptions longer than the target length are shortened with OpenAI (`gpt-5.4-mini` by default) so the full rules fit the small card back at a fixed 7pt font instead of clipping. The prompt preserves mechanics (bonuses, dice, save DCs, charges/recharge, activation, attunement effects, conditions), drops tables and "applicable weapons" lists, and keeps light flavor; named properties stay bold. Results cache to `scripts/.card-desc-cache.json` (gitignored, keyed by source hash + model + target) so rebuilds make no API calls. Flags: `--no-shorten`, `--target-chars N` (default 480), `--model NAME`. Needs `OPENAI_API_KEY`; falls back to full text (or a word-boundary trim on API error) when unset.
- **DM Command Center "Item Cards" panel (`/dm-admin#cards`).** Embeds the card generator in the website so the DM can build cards straight from the database: search the `magic_items` (4435 rows) and mundane `items` (151 rows) tables by name, add up to 9 to a tray, optionally set a footer title, and click Generate PDF to open a print-ready sheet. New admin API in `src/routes/dm-admin-api.js`: `GET /api/dm-admin/item-cards/search?q=&kind=magic|mundane|all` and `POST /api/dm-admin/item-cards`, which maps the selected DB rows to normalized card objects, writes a temp JSON, spawns `build-item-cards-pdf.js --from-json`, and streams the PDF back (501 if the script is absent, mirroring the session-PDF flow). The CLI gains a `--from-json` mode and remote-image fetching: it base64-embeds each item's `avatar_url`/`image` art, falls back to local generated art by name, or renders artless. Description shortening reuses the container's weasyprint and the app's `OPENAI_API_KEY`. Wired into the DMCC nav dropdown (`src/config.js`) and page (`src/pages/dm-admin.js`, new `cards` panel + client JS). Note: the magic-item `avatar_url` blob images currently 404 cluster-wide, so most cards render artless until that image pipeline is restored.

## [3.25.1] - 2026-07-06

### Added
- **Monster import from the RAG bestiary (STEP 5).** `mapMonsterToActor` (`src/lib/foundry-actors.js`) maps the `monsters` table (4165, full 5e stat blocks + art) to dnd5e `npc` Actors: abilities + save proficiencies, HP (avg + hit-dice formula), flat AC, all movement modes, parsed senses, CR, creature type/size, and damage/condition resistances/immunities/vulnerabilities as sheet traits; the blob-hosted art becomes the token with Dynamic Token Rings. The export endpoint gains `type=monster` with **name search** (`?q=`) or exact `?id=` (capped, since 4165 is too many to dump). The Foundry module adds a `HOTD MONSTER <name>` chat command and an "Import Monster…" dialog option that search + create/update matches idempotently (capped at 12; refine if more). Monsters default to hostile disposition. Mapping verified against live rows. Conjurer's charter updated to own this on-demand path.
- **Website → FoundryVTT actor import (STEP 4, PCs + NPCs).** New export endpoint `GET /api/foundry/actors?type=pc|npc` (`src/routes/api.js`, same token+CORS as the DM AI bridge) maps `hotd_player_characters` and `hotd_npcs` to dnd5e (v13/5.2.x) `Actor.create()` payloads via `src/lib/foundry-actors.js` — abilities, HP/AC (flat), movement, skill/save proficiencies + expertise, currency, and a composed biography for PCs; narrative biography + disposition for NPCs; portraits become the token image with **Dynamic Token Rings** enabled. The `hotd-website-integration` module gains a GM import path: an **"Import from HotD" button** in the Actors sidebar plus a `HOTD IMPORT pcs|npcs` chat command; it fetches the export, resolves relative image URLs, and **creates or updates idempotently** (matched on module flags `sourceType`+`sourceId`). Monsters (4165) are deferred to on-demand. Mapping verified against live DB rows.
- **`Conjurer` Squad agent (STEP 4 kickoff)** — Actor & Token Conjurer that owns turning website/RAG data (`hotd_player_characters`, `hotd_npcs`, `monsters`) into FoundryVTT dnd5e Actors + tokens, with AI-assisted field mapping. Wired into `team.md`, `routing.md`, the Squad coordinator, and `copilot-instructions.md`. Collaborates with Ranger (stat blocks), Bard (token art), Wizard (importer code), Summoner (Foundry ops), Artificer (export API/DB).
- **DM AI in FoundryVTT chat (STEP 3).** Type `DMAI <question>` in the Foundry chat and the campaign RAG answers in-game. New website endpoint `POST /api/foundry/dmai` (in `src/routes/api.js`) runs the same RAG path as the website DM AI (`chatWithTools`), guarded by a shared bearer token + CORS (token/origin in `hotd_config` keys `foundry_dmai_token` / `foundry_dmai_origin`, so no Helm/secret changes). The `hotd-website-integration` Foundry module now registers DM-AI settings (`dmaiToken`, `dmaiTrigger`, `dmaiWhisper`, `dmaiPlayersAllowed`) and a `chatMessage` hook that intercepts the trigger, posts a "consulting the archives" note, calls the endpoint, and renders the answer. **GM-only + whispered by default** because answers use DM-only RAG (`isDM`); players can be enabled and receive player-safe answers. The module was installed + enabled in the live instance with settings pre-seeded from `hotd_config` (the operator never handles the token), and this is captured in `foundryvtt/infra/scripts/configure-instance.sh`. **Activation: push to deploy the website endpoint** (the Foundry side is already primed).
- **Configured the live FoundryVTT instance (STEP 2) + reproducible `foundryvtt/infra/scripts/configure-instance.sh`.** Installed the **dnd5e** system (`release-5.2.5`, Verified 13.351) and three v13-pinned modules — **touch-vtt** `2.3.13`, **dice-so-nice** `5.2.5`, **monks-common-display** `13.01` (14.01 is v14-only) — via Foundry's setup HTTP API (`POST /auth` adminAuth → `POST /setup` installPackage), created the **'Halls of the Damned'** dnd5e world, and enabled the three modules by writing `core.moduleConfiguration` into the world's settings LevelDB (`classic-level`, from the running pod). The script is idempotent and re-runnable (survives a PVC rebuild). **MaterialDeck** is not installed: the v13 release (v2.2.0) is premium/Patreon-gated (the public "latest" GitHub release is a stale v11 build, which was removed) — it must be added via Foundry's module browser after linking a Material Foundry Patreon account, then enabled alongside `materialdeck-dnd5e`.
...
- **FoundryVTT still showed the "License Key Activation" page on first boot** even with the key injected — passing `--licenseKey` doesn't *sign* the license (`Config/license.json`). `entrypoint.sh` now auto-signs on first boot: it waits for the server, then POSTs the key to `/license` (idempotent; skipped once `license.json` exists). The live instance was signed manually and is now active (redirects to `/setup`).
...
### Added
- **Notebook AI Assist** — the Campaign Notebook (`/dm-admin#notes`) has a **✨ AI** button that opens a modal: a freeform prompt + optional related entities generate a RAG-grounded Markdown document (`POST /api/dm-admin/notebook/generate`, grounded via `buildEmbeddingContext` + direct NPC lookups). The draft is **editable inline**, and a **follow-up "Refine with AI"** box lets you iterate on the current draft (the endpoint accepts an optional `baseContent` so refinements revise the working document rather than starting over) with a collapsible live preview. When ready, **Create Draft Page** (pick folder + name) drops it into the notebook as a draft you can edit and publish. This replaces Story Forge with an in-notebook authoring flow.
